...
// 1 handle
function countMatch(regex) {
	let count = 0;
	for(let i = 0 ; i < journalists.length ; i++) {
		var res = journalists[i].match(regex);
...
// plusieurs cas par handle
function countAllMatch(regex) {
	let count = 0;
	for(let i = 0 ; i < journalists.length ; i++) {
		var res = journalists[i].match(regex);
		if(res && res.length > 0)
...
// slice fait la copie de l'array
// ici sort fait un tri en spécifiant la méthode de comparaison (par la longueur de chaque élément)
console.log("Quels sont les 50 handles les plus courts de cette array ?");
let shortHandles = journalists.slice();
shortHandles.sort(function(a, b){
  return a.length - b.length;
});
// slice fait extraction (en supprimant le reste, d'où l'intérêt d'une copie de tableau du début)
console.log(shortHandles.slice(0,50));
